import React from "react";
import image from "../assets/news.png"

const NewsItem = ({title,description,src,url}) => {
  return (
    <div className="card bg-dark text-light mb-3 d-inline-block my-3 mx-3 px-2 py-3" style={{maxWidth:"345px"}}>
      <img src={src?src:image} style={{height:"200px",width:"325px"}} className="card-img-top"   />
      <div className="card-body">
        <h5 className="card-title">{title?title.slice(0,50):"Title here"}</h5>
        <p className="card-text">
          {description?description.slice(0,90):"News is about the current event. It is information about something that just happened."}
        </p>
        <a href={url} className="btn btn-danger">
          Read More
        </a>
      </div>
    </div>
  );
};

export default NewsItem;
